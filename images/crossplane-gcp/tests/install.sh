...
spec:
  package: ${PUBSUB_DIGEST}
  packagePullSecrets:
  - name: regcred
EOF

for provider in storage pubsub ; do
  kubectl wait --for=condition=Installed "provider/provider-gcp-${provider}" --timeout=3m
  kubectl wait --for=condition=Healthy   "provider/provider-gcp-${provider}" --timeout=5m
done

# Update the GCP family provider that was installed by the above providers as a dependency, to use our GCP family provider instead.
...
kubectl wait --for=condition=Installed provider/upbound-provider-family-gcp --timeout=3m
kubectl wait --for=condition=Healthy   provider/upbound-provider-family-gcp --timeout=5m
