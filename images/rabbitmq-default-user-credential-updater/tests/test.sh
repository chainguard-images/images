...
# Adapted from https://github.com/rabbitmq/cluster-operator/blob/main/docs/examples/vault-default-user/test.sh
...
USERNAME=$(kubectl exec vault-default-user-server-0 -n $RABBITMQ_NAMESPACE -c rabbitmq -- \
...
secrets="$(kubectl get secrets -l=app.kubernetes.io/name=vault-default-user -n $RABBITMQ_NAMESPACE -o json | jq .items)"
...
kubectl exec vault-default-user-server-0 -n $RABBITMQ_NAMESPACE -c rabbitmq -- \
...
kubectl exec vault-0 -n $RABBITMQ_NAMESPACE -c vault -- vault kv put secret/rabbitmq/config username='rabbitmq' password='pwd2'
...
with_retry "kubectl exec vault-default-user-server-0 -n $RABBITMQ_NAMESPACE -c rabbitmq -- rabbitmqctl authenticate_user rabbitmq pwd2"
