...
function TEST_basic_helm_install() {
  helm upgrade --install cg-test \
          ...
          --set prometheusOperator.image.repository=${IMAGE_REPOSITORY} \
          --set prometheusOperator.image.registry=${IMAGE_REGISTRY} \
          --set prometheusOperator.image.tag=${IMAGE_TAG}
